"""Detailed Storage diff dump for vehicle_hardpoints vs entry_2 reference.

Shows each ship's Storage section diffs — REF vs OUT — to look for structural patterns.
"""
import json
import sys
...
def main():
    ...
    target_ship = sys.argv[1] if len(sys.argv) > 1 else None
    ...
    for ref_item in ref_list:
        ...
        if cn not in out_by_cn:
            continue
        ref_storage = (ref_item.get("Hardpoints", {})
                       .get("Components", {})
                       .get("Storage", {}))
        out_storage = (out_by_cn[cn].get("Hardpoints", {})
                       .get("Components", {})
                       .get("Storage", {}))
        if not ref_storage and not out_storage:
            continue
        acc = []
        diffs(ref_storage, out_storage, "Storage", acc)
        if acc:
            by_ship[cn] = acc

    if target_ship:
        if target_ship not in by_ship:
            print(f"No storage diffs for {target_ship}")
            print(f"REF: {json.dumps(next((r for r in ref_list if r['ClassName']==target_ship), {}).get('Hardpoints', {}).get('Components', {}).get('Storage'), indent=2)[:2000]}")
            print(f"OUT: {json.dumps(out_by_cn.get(target_ship, {}).get('Hardpoints', {}).get('Components', {}).get('Storage'), indent=2)[:2000]}")
            return
        print(f"=== {target_ship} ({len(by_ship[target_ship])} diffs) ===")
        for path, kind, r, o in by_ship[target_ship]:
            print(f"\n  [{kind}] {path}")
            if r is not None:
    ...
    print(f"Ships with storage diffs: {len(by_ship)}")
    print(f"Total storage diffs: {total}\n")

    sig_counts = Counter()
    for cn, lst in by_ship.items():
        for path, kind, r, o in lst:
            # Generalize signature: replace [PortName=...] with [...]
            import re
    ...
    for cn, lst in sorted(by_ship.items(), key=lambda x: -len(x[1])):
