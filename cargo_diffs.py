"""Detailed Cargo (CargoGrids + CargoContainers) diff dump."""
import json
import re
import sys
...
def main():
    ...
    for ref_item in ref_list:
        ...
        if cn not in out_by_cn:
            continue
        ref_comp = ref_item.get("Hardpoints", {}).get("Components", {})
        out_comp = out_by_cn[cn].get("Hardpoints", {}).get("Components", {})
        for cat in ("CargoGrids", "CargoContainers"):
            ref_block = ref_comp.get(cat, {})
            out_block = out_comp.get(cat, {})
            if not ref_block and not out_block:
                continue
            acc = []
            diffs(ref_block, out_block, cat, acc)
            if acc:
                by_ship[cn].extend(acc)

    if target:
        if target not in by_ship:
            print(f"No cargo diffs for {target}")
            return
    ...
    print(f"Ships with cargo diffs: {len(by_ship)}")
    print(f"Total cargo diffs: {total}\n")
