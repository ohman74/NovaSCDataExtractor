"""Project full vehicle records into three slice shapes.

The output is split across three files matching the established
downstream layout:
- vehicle_metadata: catalog metadata (scalar Cargo, Type, external-web placeholders)
- vehicle_stats: detailed stats (object Cargo, full spec)
- vehicle_hardpoints: ports/structural (PortTags, Hull, Hardpoints)

We build one rich record per vehicle then project it into each slice.
"""
