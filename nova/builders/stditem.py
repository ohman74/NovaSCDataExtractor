...
Produces the rich item data format used downstream — Durability,
...
def _build_recoil_block(components, ctx):
    ...
    Walks weapon → ActorProceduralRecoilConfig → setups → ActorProceduralRecoilModifiers.
    Returns one entry per filter setup (typically two: ADS + hipfire) with
    {aimStance, hands, aim, body, head} blocks. Returns None when the weapon
    has no recoil config.
    """
    weapon = components.get("weapon", {}) if isinstance(components, dict) else {}
    config_guid = weapon.get("actorProceduralRecoilConfig", "")
    if not config_guid:
        return None
    config = ctx.recoil_configs.get(config_guid)
    if not config:
        return None

    setups_out = []
    for setup in config.get("setups", []):
        mod_guid = setup.get("modifiersGuid", "")
        if not mod_guid:
            continue
        mod = ctx.recoil_modifiers.get(mod_guid)
        if not mod:
            continue
        entry = {
            "AimStance": setup.get("aimStance", "Any"),
            "Stance": setup.get("stance", "Any"),
            "Pose": setup.get("pose", "Any"),
        }
        if "hands" in mod:
            entry["Hands"] = mod["hands"]
        if "aim" in mod:
            entry["Aim"] = mod["aim"]
        if "body" in mod:
            entry["Body"] = mod["body"]
...
def _build_crafting_block(record, ctx):
    ...
    Returns None when no blueprint targets this item. The blueprint cache
    is keyed by `CraftingProcess_Creation.entityClass` GUID and only
    contains creation-type recipes (skips dismantle/salvage variants).
    """
    guid = record.get("guid", "")
    if not guid:
        return None
    bp = ctx.crafting_blueprints.get(guid)
    if not bp:
        return None
    ...
    for tier in bp.get("tiers", []):
        tier_dict = {}
        ct = tier.get("craftTime")
        if ct:
            total = (ct.get("days", 0) * 86400 + ct.get("hours", 0) * 3600
                     + ct.get("minutes", 0) * 60 + ct.get("seconds", 0))
            tier_dict["CraftTime"] = {
                "Days": int(ct.get("days", 0)),
                "Hours": int(ct.get("hours", 0)),
                "Minutes": int(ct.get("minutes", 0)),
                "Seconds": int(ct.get("seconds", 0)),
        ...
        for slot in tier.get("slots", []):
            ...
            for mod in slot.get("modifiers", []):
                ...
                mods_out.append({
                    ...
                    "EndQuality": mod.get("endQuality", 0),
                })
            costs_out = []
            for cost in slot.get("costs", []):
                costs_out.append({
                    "Type": cost.get("type", ""),
                    "Reference": cost.get("guid", ""),
        ...
        tier_dict["Slots"] = slots_out
        tiers_out.append(tier_dict)
...
def _get_fps_damage_profile(components, ctx):
    ...
    Returns dict with float keys 'physical'/'energy'/'distortion'/'stun' and
...
def _fps_class_value(class_name, full_type, mfr_code, damage_profile=None,
    ...
    # Structural damage-profile classification.
    if damage_profile:
        phys = damage_profile.get("physical", 0)
        energy = damage_profile.get("energy", 0)
        distortion = damage_profile.get("distortion", 0)
        stun = damage_profile.get("stun", 0)
...
def build_std_item(record, ctx, external_loadout=None, nested=False):
    ...
    # Lifetime + full wear model from SDegradationParams (P3b — full surfacing).
    # The component is parsed generically so values come through as strings.
    ...
    if isinstance(deg, dict):
        ...
        if isinstance(wear, dict) and wear:
            ...
            wear_block = {
                "DamageConversionRate": safe_float(wear.get("DamageConversionRate", "0")),
                "AccumulationEventThreshold": safe_int(wear.get("AccumulationEventThreshold", "1")),
                "AtmosphereMultiplier": safe_float(wear.get("AtmosphereMultiplier", "1")),
                "InitialAccumulationRatio": safe_float(wear.get("InitialAccumulationRatio", "0")),
                "InitialAgeRatio": safe_float(wear.get("InitialAgeRatio", "0")),
                "InitialUsageRatio": safe_float(wear.get("InitialUsageRatio", "0")),
                "UseAsTimer": str(wear.get("UseAsTimer", "0")) == "1",
            }
            hm = wear.get("HeatMultipliers")
            if isinstance(hm, dict):
                wear_block["HeatMultipliers"] = {
    ...
    # Attach the wear model onto whatever Durability block exists.
    if wear_block and "Durability" in si:
        si["Durability"]["Wear"] = wear_block
    ...
    if "weapon" in components:
        ...
        if weapon_data:
            ...
            if is_fps:
                ...
                # P1b — ADS Aim block (zoomScale/zoomTime + optional DOF). FPS-only;
                # ship weapons use turret-level aim mechanics.
                aim = weapon_comp.get("aim")
                if aim:
                    aim_block = {
                        "ZoomScale": float(aim.get("zoomScale", 1.0)),
                        "ZoomTime": float(aim.get("zoomTime", 0.0)),
                    }
                    if aim.get("toggleZoomOverride"):
                        aim_block["ToggleZoomOverride"] = True
                    if "dof" in aim:
                        aim_block["DepthOfField"] = {
                            "FocalDistance": aim["dof"].get("focalDistance", 0.0),
                            "FocalRange": aim["dof"].get("focalRange", 0.0),
                            "FStop": aim["dof"].get("fstop", 0.0),
                        }
                    weapon_data["Aim"] = aim_block
            # P3c — Power-mode blocks (full surfacing with recoil/aim/spread/regen).
            # Applies to both ship and FPS weapons.
            pm = {}
            for stats_name, label in (("noPowerStats", "NoPower"),
                                        ("underpowerStats", "UnderPower"),
                                        ("overpowerStats", "OverPower"),
                                        ("overclockedStats", "Overclocked")):
                block = weapon_comp.get(stats_name)
                if block:
                    pm[label] = block
            if pm:
                weapon_data["PowerModes"] = pm
            # P2d/P2e — Heat: lockOnOverheat, heatReduceWhenFixed, temperatureCurve axes.
            # Mutates HeatParameters in place if it exists, else only creates one when
            # we have something to add.
            heat_block = weapon_data.get("HeatParameters") or {}
            heat_changed = False
            if "lockOnOverheat" in weapon_comp:
                heat_block["LockOnOverheat"] = bool(weapon_comp["lockOnOverheat"])
                heat_changed = True
            if "heatReduceWhenOverheatIsFixed" in weapon_comp:
                heat_block["HeatReduceWhenFixed"] = float(weapon_comp["heatReduceWhenOverheatIsFixed"])
                heat_changed = True
            shp = weapon_comp.get("simplifiedHeat") or {}
            if "temperatureCurveAxes" in shp:
                heat_block["TemperatureCurveAxes"] = shp["temperatureCurveAxes"]
                heat_changed = True
            if heat_changed and "HeatParameters" not in weapon_data:
                weapon_data["HeatParameters"] = heat_block
            ...
            if "Aim" in weapon_data:
                ordered["Aim"] = weapon_data["Aim"]
            ...
            if "PowerModes" in weapon_data:
                ordered["PowerModes"] = weapon_data["PowerModes"]
            si["Weapon"] = ordered

        recoil = _build_recoil_block(components, ctx)
        if recoil:
            si["Recoil"] = recoil

    crafting = _build_crafting_block(record, ctx)
    if crafting:
        si["Crafting"] = crafting
    ...
    if ifcs_params:
        ...
        if ifcs:
            _apply_blade_modifier(ifcs, ifcs_params, ctx)
        si["Ifcs"] = ifcs
...
def _build_weapon_modifier(weapon_mod):
    ...
    Maps XML `modifier.weaponStats` tree to the output format:
...
def _build_weapon_data(components, ctx, item_type="", is_fps=False):
    ...
    if firing_modes:
        ...
        for mode in firing_modes:
            ...
            # P1c — post-shot cooldown (XML @cooldownTime). For non-charged
            # modes only; charged modes emit it via FireChargedParameters.Cooldown.
            cd = mode.get("cooldownTime")
            if cd is not None and fire_type != "charged":
                fm["CooldownTime"] = float(cd)

            # P1a — per-firing-mode procedural recoil curves
            # (WeaponProceduralRecoilConfigDef, resolved via @recoil GUID).
            rcfg_guid = mode.get("recoilConfig")
            if rcfg_guid:
                rcfg = ctx.weapon_recoil_configs.get(rcfg_guid)
                if rcfg:
                    rec_block = {}
                    if rcfg.get("hands"):
                        rec_block["Hands"] = rcfg["hands"]
                    if rcfg.get("aim"):
                        rec_block["Aim"] = rcfg["aim"]
                    if rcfg.get("body"):
                        rec_block["Body"] = rcfg["body"]
                    if rcfg.get("head"):
                        rec_block["Head"] = rcfg["head"]
                    if rec_block:
                        fm["Recoil"] = rec_block

            # P3a — Misfire (jam mechanics) per fire mode.
            mf_guid = mode.get("misfire")
            if mf_guid:
                mf = ctx.misfire_defs.get(mf_guid)
                if mf:
                    fm["Misfire"] = {
                        "MinorDuration": mf.get("minorDuration", 0.0),
                        "MinorCooldown": mf.get("minorCooldown", 0.0),
                        "MajorCooldown": mf.get("majorCooldown", 0.0),
                    }
    ...
    if ammo_data:
        ...
        # P2a — projectile physics (bullet drop)
        phys = ammo_data.get("projectilePhysics")
        if phys:
            ammo["Projectile"] = {
                "Mass": phys.get("mass", 0.0),
                "AirResistance": phys.get("airResistance", 0.0),
                "DisableGravity": phys.get("disableGravity", False),
                "Radius": phys.get("radius", 0.0),
                "Pierceability": phys.get("pierceability", 0),
            }

        # P2b — impulse / force reaction
        imp_scale = ammo_data.get("impulseScale")
        imp_falloff = ammo_data.get("impulseFalloff")
        if imp_scale is not None or imp_falloff:
            ammo["Impulse"] = {"Scale": float(imp_scale) if imp_scale is not None else 1.0}
            if imp_falloff:
                ammo["Impulse"]["Falloff"] = {
                    "MinDistance": imp_falloff.get("minDistance", 0.0),
                    "DropPerMeter": imp_falloff.get("dropFalloff", 0.0),
                    "MaxFalloff": imp_falloff.get("maxFalloff", 0.0),
                }

        # P2c — armor-tier penetration falloff
        pierce = ammo_data.get("pierceability")
        if pierce:
            ammo["ArmorPenetration"] = {
                "FalloffLevel1": pierce.get("level1", 0.0),
                "FalloffLevel2": pierce.get("level2", 0.0),
                "FalloffLevel3": pierce.get("level3", 0.0),
            }
            mpt = ammo_data.get("maxPenetrationThickness")
            if mpt:
                ammo["ArmorPenetration"]["MaxThickness"] = float(mpt)
    ...
    for entry in default_loadout:
        ...
        if ammo_data:
            ...
            ammo = {
                ...
                "Capacity": float(mag_ammo.get("maxAmmoCount", 0)),
            }
            exp_min = ammo_data.get("explosionRadiusMin")
            exp_max = ammo_data.get("explosionRadiusMax")
            ...
            # P2a — Projectile physics (bullet drop calc inputs)
            phys = ammo_data.get("projectilePhysics")
            if phys:
                ammo["Projectile"] = {
                    "Mass": phys.get("mass", 0.0),
...
def _apply_blade_modifier(ifcs, ifcs_params, ctx):
    """Apply IFCS modifier deltas from a SIFCSModifiersLegacy record.

    The item's `IFCSParams.modifiersLegacy` holds a GUID pointing at a
    `SIFCSModifiersLegacy` record. The record's positional `numbers` and
    `vectors` are the actual delta values to apply. CIG positional convention
    (verified 2026-05-06 against FlightBlade_HND / FlightBlade_SPD records):

    - `numbers[0]` (Additive)  -> SCMSpeed delta
    - `numbers[1]` (Additive)  -> BoostSpeedForward delta
    - `numbers[2]` (Additive)  -> BoostSpeedBackward delta
    - `numbers[3]` (Additive)  -> MaxSpeed delta
    - `vectors[0]` (Additive)  -> AngularVelocity delta:
        x -> Pitch, y -> Roll, z -> Yaw
    - `vectors[1..4]` (Scalar) -> capacitor/scaling curves not surfaced in
       the Ifcs block; ignored.

    Replaces the previous className-suffix HND/SPD heuristic + hardcoded
    constants. Future-proof against new modifier records (e.g. balanced
    variants) — the deltas come straight from the record.
    """
    if not isinstance(ifcs_params, dict) or not ctx:
        return
    modifier_guid = (ifcs_params.get("modifiersLegacy") or "").strip()
    if not modifier_guid or modifier_guid == "00000000-0000-0000-0000-000000000000":
        return
    record = (getattr(ctx, "ifcs_modifiers", None) or {}).get(modifier_guid)
    if not record:
        return

    nums = record.get("numbers") or []
    vecs = record.get("vectors") or []

    def _add(num_idx, target_dict, key):
        if num_idx >= len(nums):
            return
        n = nums[num_idx]
        if not isinstance(n, dict) or n.get("type") != "Additive":
            return
        if key in target_dict:
            target_dict[key] = target_dict[key] + n.get("value", 0)

    _add(0, ifcs, "SCMSpeed")
    _add(3, ifcs, "MaxSpeed")
    mm = ifcs.get("MasterModes")
    if isinstance(mm, dict):
        _add(1, mm, "BoostSpeedForward")
        _add(2, mm, "BoostSpeedBackward")

    av = ifcs.get("AngularVelocity")
    if isinstance(av, dict) and vecs and isinstance(vecs[0], dict) and vecs[0].get("type") == "Additive":
        v0 = vecs[0]
        if "Pitch" in av:
            av["Pitch"] = av["Pitch"] + v0.get("x", 0)
        if "Roll" in av:
            av["Roll"] = av["Roll"] + v0.get("y", 0)
        if "Yaw" in av:
            av["Yaw"] = av["Yaw"] + v0.get("z", 0)
...
def _build_modifier(si, modifier_params, item_type):
    ...
    # VehicleMod mining modules (type "UNDEFINED.Gun") emit 3-entry buff modifier arrays
    ...
    if "Salvage" in item_type:
        ...
        # REF emits when filterModifier has a value, OR when modifierLifetime has a
        # lifetime (active/timed modules emit even with 0). Passive non-filter modules
        # have empty filterModifier AND no lifetime — skip those.
        ...
        if isinstance(mfip, dict) and mfip:
            ...
            ml = mfip.get("modifierLifetime", {})
            has_lifetime = isinstance(ml, dict) and "ItemModifierTimedLife" in ml
            if has_filter_value or has_lifetime:
