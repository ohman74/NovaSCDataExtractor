...
**Badge**: Downstream consumers of these output files should display the official "Powered by UEX" badge wherever shop prices are shown, linked to https://uexcorp.space.
...
- Recommended markup:
  ...
  ```
- Downstream apps may mirror the badge locally (e.g. copy into a local `img/` directory) to avoid hotlinking, but the `<a href>` should still point to uexcorp.space.
